...
- **Navigates before it plans (agentic loop).** With a tool-capable model, the agent can call tools — `read_file`, `grep`, `list_dir`, `run_command` — to inspect the real repository before proposing its plan, instead of guessing from metadata. Reads are workspace-confined and secret-filtered. Providers without tool support transparently fall back to schema-constrained one-shot planning.
- **Pursues the tools the task needs.** The agent isn't limited to a fixed command allow-list. When a run is approved (`--yes` / interactive approval / autonomous), it may run any **clean** command it needs to get the job done — install a dependency, invoke a linter or generator, run a script, use another toolchain — and a plan's validation/setup commands work the same way. The gate is one-directional: a hard floor of commands is **never** run regardless of approval — `sudo`, `curl`/`wget` (no raw network egress), `ssh`/`scp`, `rm -rf /`, `chmod 777`, `chown`, and any shell-metacharacter/`$()` injection. Plugins remain allow-list-only.
