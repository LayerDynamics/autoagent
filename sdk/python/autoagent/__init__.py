...
def replay(root: str, session_id: str, *, approve: bool = False) -> RunOutcome:
    """Deterministically replay a recorded session, reproducing the run."""
    return RunOutcome.from_dict(json.loads(_call(_native.replay, root, session_id, approve)))


def run_sync(root: str, objective: str, from_: Optional[str] = None, *, approve: bool = False) -> RunOutcome:
    """Supervised run (blocking) from a plan or generated objective."""
    return RunOutcome.from_dict(json.loads(_call(_native.run_sync, root, objective, from_, approve)))


def evolve_sync(root: str, objective: str, from_: Optional[str] = None, *, apply: bool = False) -> EvolveOutcome:
    """Controlled self-authoring (blocking)."""
    return EvolveOutcome.from_dict(json.loads(_call(_native.evolve_sync, root, objective, from_, apply)))


async def run(root: str, objective: str, from_: Optional[str] = None, *, approve: bool = False) -> RunOutcome:
...
async def evolve(root: str, objective: str, from_: Optional[str] = None, *, apply: bool = False) -> EvolveOutcome:
